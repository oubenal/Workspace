...
{
    // NOTE: this type is provided as a prerequisite for Demo 4; see Demo 3 for its definition
    ...
    public class SingleStatementBodyAnalyzer : ISyntaxNodeAnalyzer<SyntaxKind>
    {
        ...
        private static DiagnosticDescriptor desc = new DiagnosticDescriptor(ID, "Single statement bodies should be surrounded by curly braces.", "'{0}' should use curly braces around the statement body.", "Naming", DiagnosticSeverity.Warning, true);
        private static ImmutableArray<DiagnosticDescriptor> descs = ImmutableArray.Create(desc);
        private static ImmutableArray<SyntaxKind> kinds = ImmutableArray.Create(SyntaxKind.IfStatement, SyntaxKind.ElseClause, SyntaxKind.WhileStatement, SyntaxKind.ForStatement, SyntaxKind.ForEachStatement);
